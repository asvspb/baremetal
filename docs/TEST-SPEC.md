...
- M-3: `templates/unattend.xml.template` — well-formed (`xmllint --noout`), содержит `<FirstLogonCommands>`, `AllowTelemetry` и `Remove-AppxProvisionedPackage`; без хрупких счётчиков Order.
