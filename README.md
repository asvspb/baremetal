...
> ℹ️ **Контроль свежести копии на Windows:** скрипт выводит версию сборки в баннере и в шапке лога (`Версия скрипта: 2026-09-02.1`). Перед запуском сверьте её со значением `$ScriptVersion` в актуальном файле репозитория — несовпадение означает устаревшую копию на машине. Точная сверка — хэшем: `Get-FileHash .\make-boot-usb.ps1` (Windows) против `sha256sum make-boot-usb.ps1` (репозиторий). При каждой правке скрипта версия в шапке обновляется (см. `AGENTS.md`).
...
> **Про замеры скорости в make-boot-usb:** скорость **записи** — честный холодный
> замер носителя. Скорость **чтения** помечена «(из кэша ОС)»: тест читает файл,
> только что записанный самим тестом, и ОС отдаёт его из кэша — число отражает
> производительность конвейера ОС, а не флешки. (Холодный замер чтения через
> `FILE_FLAG_NO_BUFFERING` — отложенное улучшение.) В Linux-версии кэш сбрасывается
> через `drop_caches`, там замер чтения честный.
