...
- [ ] **T1.1 (S)** 🔴 Удалить дубликат в `make-boot-usb.ps1`: оставить строки 1–1120,
...
- [ ] **T1.2 (M)** 🔴 `make-boot-usb.sh`: безопасный бэкап:
...
- [ ] **T1.3 (M)** 🔴 `make-boot-usb.ps1`: то же самое: `Copy-Item` без
...
- [ ] **T2.1 (L)** 🔴 Стратегия установки Windows — **решение: вариант A**
...
- [ ] **T4.1 (M)** Скрипт `check-files.sh`: проверка BOM+CRLF+один SYNOPSIS+
